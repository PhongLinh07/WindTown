...
Public Class Level_List_UC
    Inherits BaseList_UC

    Public Sub New()
        MyBase.New(New BaseService(Of Level), "Level")
        InitializeComponent()
        Init(GetType(Level))
    End Sub
    ...
    Protected Overrides Sub Dgv_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs)
        ...
        Dim data = Utils.DeepClone(CType(row.DataBoundItem, Level))

        Dim crud As New Level_CRUD_Frm(data)
    ...
    Protected Overrides Sub tool_new_Click(sender As Object, e As EventArgs)
        ' Tạo bản sao của đối tượng để tránh sửa trực tiếp trên DataGridView
        Dim data = New Level() ' tạo mới đối tượng với giá trị mặc định

        Dim crud As New Level_CRUD_Frm(data, True)
        If crud.ShowDialog() = DialogResult.OK Then
    ...
    Protected Overrides Sub tool_delete_Click(sender As Object, e As EventArgs)
        ...
        Dim items As List(Of Level) = _dgv.SelectedRows.
                                                Cast(Of DataGridViewRow)().
                                                Select(Function(r) TryCast(r.DataBoundItem, Level)).
