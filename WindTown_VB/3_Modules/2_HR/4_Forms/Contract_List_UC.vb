Public Class Contract_List_UC
    Inherits BaseList_UC

    Public Sub New()
        MyBase.New(New ContractService(), "Contract")
        InitializeComponent()
    ...
    Protected Overrides Sub Dgv_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs)
        ...
        Dim data = Utils.DeepClone(CType(row.DataBoundItem, Contract))

        Dim crud As New Contract_CRUD_Frm(data)
    ...
    Protected Overrides Sub tool_new_Click(sender As Object, e As EventArgs)
        ' Tạo bản sao của đối tượng để tránh sửa trực tiếp trên DataGridView
        Dim data = New Contract() ' tạo mới đối tượng với giá trị mặc định

        Dim crud As New Contract_CRUD_Frm(data, True)
        If crud.ShowDialog() = DialogResult.OK Then
    ...
    Protected Overrides Sub tool_delete_Click(sender As Object, e As EventArgs)
        ...
        Dim items As List(Of Contract) = _dgv.SelectedRows.
                                                Cast(Of DataGridViewRow)().
                                                Select(Function(r) TryCast(r.DataBoundItem, Contract)).
