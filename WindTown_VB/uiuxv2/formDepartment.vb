...
Public Class formDepartment
    ...
    ' ============================================================
    '  formDepartment.vb — Phòng ban  (.NET 8 · DB qua DepartmentSV)
    ' ============================================================

    Private ReadOnly _deptSv = AppServices.Instance.DepartmentSV

    Private _all As New List(Of Department)()
    Private _filtered As New List(Of Department)()
    Private _selId As Integer = -1

    Private Shared Function JobCount(d As Department) As Integer
        Return If(d.Jobs IsNot Nothing, d.Jobs.Count, 0)
    End Function
    ...
    Private Sub LoadData()
        Dim res = _deptSv.GetList()
        If Not res.IsSuccess Then
            MessageBox.Show(res.Message, "Lỗi tải dữ liệu", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            _all = New List(Of Department)()
        Else
            _all = CType(res.Data, IEnumerable(Of Department)).ToList()
        End If
        _filtered = New List(Of Department)(_all)
    ...
    Private Sub RenderTable(data As List(Of Department))
        dgv.Rows.Clear()
        For Each r In data
            Dim jc = JobCount(r)
    ...
    Private Sub SelectItem(id As Integer)
        ...
        btnDelete.Enabled = (JobCount(r) = 0)
    End Sub
    ...
    Private Sub ApplyFilter()
        ...
        _filtered = New List(Of Department)()
        For Each r In _all
    ...
    Private Sub btnSave_Click(s As Object, e As EventArgs) Handles btnSave.Click
        ...
        If _selId <= 0 Then
            If _deptSv.IsCodeDuplicate(code, 0) Then
                MessageBox.Show("Mã phòng ban đã tồn tại.", "Trùng mã", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                txtFCode.Focus() : Return
            End If
            Dim d As New Department()
            d.code = code
            d.name = txtFName.Text.Trim()
            d.note = txtFNote.Text.Trim()
            d.status = If(cboFStatus.SelectedIndex = 0, 1, 0)
            Dim res = _deptSv.Insert(d)
            If res.IsSuccess Then
                MessageBox.Show(If(String.IsNullOrEmpty(res.Message), "Đã thêm phòng ban.", res.Message), "Thành công", MessageBoxButtons.OK, MessageBoxIcon.Information)
                _selId = d.id
                LoadData()
            Else
            ...
            End If
        Else
            If _deptSv.IsCodeDuplicate(code, _selId) Then
                MessageBox.Show("Mã phòng ban đã tồn tại.", "Trùng mã", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                txtFCode.Focus() : Return
            End If
            ...
            Dim res = _deptSv.Update(existing)
            If res.IsSuccess Then
                MessageBox.Show(If(String.IsNullOrEmpty(res.Message), "Đã cập nhật phòng ban.", res.Message), "Thành công", MessageBoxButtons.OK, MessageBoxIcon.Information)
                LoadData()
            Else
                MessageBox.Show(res.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
    ...
    Private Sub btnDelete_Click(s As Object, e As EventArgs) Handles btnDelete.Click
        ...
        If r Is Nothing Then Return
        If JobCount(r) > 0 Then
            MessageBox.Show("Không thể xóa phòng ban đang có chức danh (công việc) liên kết.", "Không thể thực hiện", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If
        Dim ask = MessageBox.Show("Ngừng sử dụng phòng ban """ & r.name & """?", "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Warning)
        If ask <> DialogResult.Yes Then Return
        Dim res = _deptSv.Delete(New List(Of Department) From {r})
        If res.IsSuccess Then
